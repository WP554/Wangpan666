...
def generate_wordcloud(word_counts):
    wordcloud = WordCloud(font_path='Font/SimHei.ttf', width=800, height=400,
...
def plot_matplotlib(freq_df, chart_type):
    ...
    elif chart_type == "箱线图":  # Box plot
        plt.boxplot(freq_df['频率'], labels=['频率'])
        plt.title('箱线图')
        plt.ylabel('频率')

    elif chart_type == "热力图":  # Heatmap
        # Create a simple heatmap using frequencies
        heatmap_data = pd.DataFrame(freq_df['频率']).T
        plt.imshow(heatmap_data, cmap='hot', interpolation='nearest')
        plt.title('热力图')
        plt.colorbar()
        plt.xticks(range(len(freq_df)), freq_df['词语'], rotation=45)
...
def plotly_visualizations(freq_df, chart_type):
    ...
    if chart_type == "柱状图":
        ...
        fig = px.scatter(freq_df, x='词语', y='频率', title='Plotly散点图', color='频率', hover_name='词语')
        ...
        fig = px.box(freq_df, y='频率', title='Plotly箱线图', points="all")

    elif chart_type == "热力图":
        heatmap_data = pd.DataFrame(freq_df['频率']).T
        fig = px.imshow(heatmap_data, title='Plotly热力图', color_continuous_scale='Viridis')

    fig.update_layout(font=dict(family='Arial, "微软雅黑", "宋体", "SimHei"', size=14))
    return fig
...
def altair_visualizations(freq_df, chart_type):
    ...
    if chart_type == "柱状图":
        ...
        chart = alt.Chart(freq_df).mark_boxplot().encode(y='频率', x='词语')
    elif chart_type == "热力图":
        # Using a matrix of data points, we assume different words map to different frequencies
        heatmap_data = pd.DataFrame(freq_df['频率']).T
        chart = alt.Chart(heatmap_data).mark_rect().encode(
            x=alt.X('0:Q', axis=alt.Axis(title='词语')),
            y=alt.Y('0:Q', axis=alt.Axis(title='频率')),
            color='0:Q'
        )
    ...
    if st.sidebar.button("抓取并分析"):
        ...
        try:
            ...
            if visualization_type == "Matplotlib":
                if chart_type == "词云图":
                    plt_fig = generate_wordcloud(word_counts)
                    st.pyplot(plt_fig)
                else:
                ...
                if chart:  # Make sure it's not None for 热力图
